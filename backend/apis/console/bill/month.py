...
def classify_fee_type(bill: MonthBill) -> str:
    """
    根据产品类型粗略区分费用类型：
    - 流量 / 短信等按量计费视为 Token 使用费
    - 计算类（云电脑、ECS、GPU、裸金属等）视为月租费
    - 其余归为其他费用
    """
...
@router.get(tags=[APITags.console], summary="查月账单明细（原始记录）")
async def _(
...
@router.get(tags=[APITags.console], summary="查月账单汇总（按月一张账单）", path="/console/bill/month/summary")
