...
from pydantic import BaseModel, Field, constr, field_validator, create_model, ConfigDict
...
class ValuedField(BaseModel):
    ...
    none_placeholder: constr(strip_whitespace=True) = Field(default="", description="针对None值的占位")
...
class RedisKeyTemplate(BaseModel):
    """redis键模板"""
    ...
    _constructed: Type[TemplateValues] | None = None
    ...
    def template_value_pdm(self) -> Type[TemplateValues]:
        if not self._constructed:
            self._constructed = create_model(
                f"anonymous-redis-key-template-values-{uuid.uuid4().hex}",
                __base__=(TemplateValues,),
    ...
    def generate(self, **valued_keys) -> str:
        """输出key"""
        values = self.template_value_pdm(**valued_keys).model_dump()
...
async def script():
    ...
    print(t3.generate(zz="xixi:", b="ceshi", appended_integer_field=321))
    print("-")
    print(t3.parse("a:{zz}:1:emmmm:{b}1:appendix:"))
