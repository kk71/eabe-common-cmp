<template>
  <div ref="keywordChart" :style="{ height: props.height, width: props.width }" />
</template>
...
<script setup>
  ...
  const keywordChart = useTemplateRef('keywordChart');

  const initKeywordChart = async () => {
    await nextTick();
    const chart = echarts.init(keywordChart.value);
    chart.setOption({
      series: [
        {
          type: 'wordCloud',
          ...
          textStyle: {
            ...
            color: function () {
              // Random color
              return (
                'rgb(' + [Math.round(Math.random() * 200), Math.round(Math.random() * 200), Math.round(Math.random() * 200)].join(',') + ')'
              );
            },
          },
          emphasis: {
